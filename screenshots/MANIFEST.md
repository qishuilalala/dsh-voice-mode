...
| ID | 标题 | 步骤 | 预期 | 工具 | 状态 |
|---|---|---|---|---|---|
| S01 | 安装成功 + config 7 字段 | 1 | `curl /voice-mode/config` 返回 asrHotwords / asrHotwordsScore / recognitionLanguage / senseITN / captionFontSize / captionMaxWidth / backchannelYield 全部非 null | Playwright + curl | 模板 + 实测可跑 |
| S02 | 设置面板中文标签 | 2 | 「识别热词」「识别语种」「字幕字号」「字幕宽度」等 7 项中文标签 | Playwright | 模板 + 实测可跑 |
| S03 | 热词即时生效 partial | 3 | partial 显示 `dsh-voice-mode` | Playwright + 麦克风 | 模板 + 待真机 |
| S04 | 锁 en 不抖回中文 | 4 | final 100% 英文 | Playwright + 麦克风 | 模板 + 待真机 |
...
- **S01**：脚本内置 Node 原生 `fetch` 先请求 `${DSH_URL}/voice-mode/config`，校验 7 字段类型非 null 后打印结果，再对页面截屏。
...
node screenshots/scripts/capture.mjs --all           # 全部 12 张（待真机的会跳过并提示）
