...
| 4 ADR-0007 步1 | 放行 | — | **前置门已满足**（docfix `a96acd9` 验收 pass，tsc×2/build/npm test 全绿）。开工按计划 §6 ADR-0007 第一步：本地引擎情感标签（emotion.ts 新文件 + tts-local 后处理 + tapActiveStream 不动 + tts-queue 完全不动——关键设计决策：emotion 处理全收在 tts-local.synthesize 内部，pump 与帧协议零触碰，I4/I5 天然无风险）。whisper 单标签→成对标签的偏差须写入 commit message 并回写 ADR-0007 落地注记。 |
| 5 ADR-0008 P1 | 待 | — | R4b 短应答边界已声明 |
| 6 总收口 | 待 | — | 真机冒烟含**热词识别效果**（批 1 P1 遗留）+ **语种切换 worker 重建**（批 2 §4.4 集成层豁免）+ **字幕 24px 多行 + 中文换行**（批 3 §5.2 真机验收项）|