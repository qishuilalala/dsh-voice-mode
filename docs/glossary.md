...
| 实例 | 阈值 | 用途 | 位置 |
|---|---|---|---|
| 端点 VAD | 0.5（保守） | 断句：静音 ≥ silenceMs（默认 1.5s）判一句说完 | `src/asr-host.ts` `newVad` |
| 检测 VAD | 0.35（灵敏） | 打断：抓人声前沿 | `src/asr-host.ts:330` |

两者不共享状态——朗读期的回声若混入端点 VAD 会污染断句。

**端点（endpoint）**
"这句说完了"的判定时刻。默认由端点 VAD 给出，可被语义确认窗口延后（见下）。客户端静音计时（`silenceMs`，默认 1500）是 VAD 缺失时的兜底。
...
匹配 = startsWith 快路径 + 容错慢路径（issue #10：lead ≤3 前导窗口 + Levenshtein ≤1，吸收同音字/首字错/前置语气词；单字词不走慢路径）。
待机态显示实时转写（唤醒成败可自查）；待机段静音 ≥ silenceMs 即弃段（防段首毒化）；每句断句/打断后回待机需重说唤醒词；打断门控是 VAD，与唤醒词无关。
**命中不丢段**（2026-09-18 真机修复）：命中时保留待机段与 host 流继续识别——用户常把命令与唤醒词连说（「你好小D你给我说100个字」），丢段会吃掉命令前缀（只剩尾部几个字）；唤醒词改由定稿时 `stripWakePrefix` 从文本头部剥掉（找不到就原样保留，保内容优先），实时字幕同步剥离。待机段音频组装与正常聆听路径同构（朗读期不入段 + prePad 补起始音 + 尾随静音照常上传）。
