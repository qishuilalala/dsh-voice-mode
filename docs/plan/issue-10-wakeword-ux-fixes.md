...
## 六、后续真机排查（2026-09-18，唤醒词仍几乎不可用）

用户复测反馈：设唤醒词后「喊不醒」→ 修完仍报「说『你好小李你给我说100个字的中文』只发出去最后几个字」。
新建 `test/wake-flow.test.mjs`（假浏览器 + 假 host 驱动**真 `asr.ts` 引擎**：可控时钟、幅度编码角色、
200ms 块转字、LAG 模拟流式滞后、定稿尾垫 0.5s）后逐项定位，共四处流程根因（D1-D4）：

| # | 缺陷（file:line 基线） | 台架证据 | 症状 |
|---|---|---|---|
| D1 | `handleAudio` wake 分支无 `isPlaying` 自聊守卫（正常路径有） | `echoFed=23552`、host 文本 `"我明白了我明白你好小"` | 朗读后喊不醒（唤醒词被挤到段中） |
| D2 | wake 分支只 push `rms>SPEECH_RMS` 帧（无 prePad、丢静音） | `recvMs=832`、`pending=[你,好,小,李]` 但 `lastText="你好"` | 4 字词只剩 2 字 → 编辑距离 2 > 1 → 失配 |
| D3 | 命中分支 `segment=[]`+`resetHostStream()` | 定稿 `"给我说一百个你"` | 连说时命令前缀被丢 → 只剩尾部几个字 |
| D4 | 命中后未置 `speechActive`（段留但无定稿判据） | LAG=3000ms 时 **0 条定稿** | 命中落在停口之后 → 命令悬挂不发 |

**修法**（详见 `docs/rules/STATE.md` 两行登记与 commit `c5975a1` / `4df38b8` + 本批）：
待机段音频组装与正常聆听路径同构（朗读期不入段并清残留 / prePad 补起始音 / 开口后尾随静音照常入段 /
命中不丢段 + 置 `speechActive` + 计入 `speechMs`）；定稿与字幕用 `stripWakePrefix` 从文本头部剥掉唤醒词
（找不到则原样保留——保内容优先）；待机段清空统一走 `clearWakeSegment()`。

**验证**：每处修复均做反向验证（回退 `src/` → 用例真红）；全量 372/372 exit 0 + tsc×2 零错。

### 已知边界（非缺陷，已文档化）

- `hold` 模式与 `bargeInMode='manual'`（含 `detect` 在本机无原生 AEC 时自动落 manual）下，`handleAudio`
  顶部就丢弃非按压帧 → **唤醒词静默失效**（已在设置文案、README 特性行与排障表写清）。
- 唤醒词以白名单语气词开头（如「那个小李」）时，归一化会把词头剥掉，导致同词但无语气词的说法也能唤醒
  （过触发方向），且纯语气词唤醒词（如「这个」）永远不命中——建议改用非语气词开头的 3-4 字词。
- 唤醒词只在非播放期生效；朗读期打断仍由 VAD 决定（开口即打断）。
- 每句断句/打断后回待机需重说唤醒词（现状语义，已有台架钉测；是否需要「会话窗口」待用户拍板）。
