# ADR-0002：打断 VAD 下沉到客户端，host VAD 退为兜底
...
判定语义（confirmFrames、回声门控、迟滞衰减）**原样保留**，只换执行位置——这样才能用 ADR-0004 的基准证明行为等价。
...
**必须先有 [ADR-0004](0004-acoustic-regression-harness.md) 的回归基准**，否则无法证明搬迁后行为等价——这类改动最典型的失败模式是"延迟降了，误打断悄悄涨了"。

## 备选方案

- **A：保持 host VAD，改用 WebSocket 传输**（[ADR-0003](0003-realtime-transport.md)）——去掉轮询量化，但保留往返，延迟改善有限（~50–80ms），收益远小于本方案，但成本也小得多
