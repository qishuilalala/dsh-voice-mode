...
| 层 | 实现 | 文件:行 |
|---|---|---|
| 物理层（声学） | 录音 → VAD → 端点检测 → 硬打断 | `src/asr.ts:640-695`（长静音检测）+ `src/client.tsx:1395-1505`（hardBreak） |
...
- **真机 backchannel 检测 = 0** — 用户插嘴"嗯/对/so"，模型继续说完所有句子
...
- `src/client.tsx:1395-1505`（hardBreak 现有执行点）
