...
- **Wake word (optional, off by default)**: after setting `wakeWord`, entering voice mode starts in standby, and recognition only begins once the wake word is spoken (e.g. `你好小D`), preventing accidental triggers. Matching is fault-tolerant (homophone substitutions, a wrong first char, and leading fillers are absorbed); in standby the status bar live-shows "say '<word>' · what it heard", so wake success/failure is self-diagnosable. Note: the wake word only gates "start of recognition" — **after each utterance split or a barge-in the engine returns to standby and the wake word must be repeated**; barge-in itself is VAD-based (any speech interrupts) and unrelated to the wake word
...
| `wakeWord` | empty (off) | Wake word (e.g. `你好小D`): speak it after entering to activate, avoiding accidental triggers; empty = off. Fault-tolerant matching (edit distance ≤1 + a 3-char leading window absorbs homophones/fillers); **use at least 2 characters** (single-char words get no tolerance); must be repeated after each utterance split or barge-in; toggle mode only |
