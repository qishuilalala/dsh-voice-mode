...
- **长段支持**：按住说整段上限 10 分钟（停顿不断句）；持续聆听单句上限 3 分钟；静音断句默认 5 秒；
...
- **两种交互模式（输入框旁按钮或设置可切换，切换即持久化）**：
  - `toggle`（默认）持续聆听：RMS VAD 分段 → paraformer 流式识别（边说边出字，实时字幕预览）→ 静音 5 秒自动断句进草稿并自动发送；按住 `Ctrl` 强制立即发送
...
| 直接说话 | `toggle`：边说边出字，停顿 5 秒自动发送；按住 `Ctrl` 强制立即发送 |
