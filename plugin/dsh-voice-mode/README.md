...
| `asrHotwords` | 空（关） | 识别热词偏置（批 1 P0）：每行一词或「词:分数」（如 `dsh-voice-mode:2.5`）；变更触发 recognizer 重建，下次进入语音模式生效；空 = 关闭 |
| `asrHotwordsScore` | `1.5` | 热词基准偏置分（sherpa-onnx 官方一致；越大越强，建议 1.5-2.5） |
| `recognitionLanguage` | `auto` | 批 2 P0：SenseVoice 多语种识别（`auto` / `zh` / `en` / `ja` / `ko` / `yue`）；切换终止并重建 worker 线程 |
