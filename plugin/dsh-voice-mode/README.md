...
- **开口打断（barge-in）**：**NLMS 回声消除（P3，512ms 窗口）**以本页 TTS 播放为参考消掉外放回声后，**host 端 Silero VAD 帧级检测（isSpeech）随 partial 下行驱动打断**——三档灵敏度对应发声确认时长（0 高门槛约 300ms / 1 中 200ms / 2 低 100ms），仅 AI 朗读中触发 → 本地静音 + host 合成队列作废 + 正在运行的回合取消（保留半截并自然续入新消息）
...
| `interruptLevel` | `0` | 发声打断灵敏度：0 高门槛 / 1 中 / 2 低（发声确认约 300/200/100ms） |
