...
> 注意：dsh 安装的是 pnpm `file:` 链接（目录拷贝），改完 `node build.mjs` 后需把 `lib/client.js` 同步到 `<profile>/node_modules/dsh-voice-mode/lib/` 再刷新页面（`lib/index.js` 与工作区为同一文件自动同步）。集成探测脚本（`test/hold-e2e.js`、`test/capture-e2e.js`、`test/asr-e2e.js`、`test/output-e2e.js`、`test/make-real-voice.mjs` + `test/sherpa-direct.js`）位于仓库根 `test/`，不在 npm 包内；`capture-e2e.js` 用 fake 麦克风做「采集→partial→final」闭环自检，`make-real-voice.mjs` 合成真实中文语音验证识别（词级对照）。
